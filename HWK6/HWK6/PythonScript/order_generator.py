...
def generate_order(fake,maxid):
    ...
    num_items = random.randint(1, min(len(all_ids), 5))  # Limit the number of items by the available IDs
    items = [{'id': all_ids.pop(), 'qty': random.randint(1,3)} for i in range(num_items)]
